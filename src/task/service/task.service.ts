...
import { CreateTaskParams, EditTaskParams } from '../../task/utils/type';
...
export class TaskService {
    ...
    async findSubmissionByTaskId(taskId: number) {
    ...
    async editedTask(taskId: number, editTask: EditTaskParams) {
        const task = await this.taskRepository.findOne({
            where: { taskId: taskId },
        });
    ...
    async deleteTask(taskId: number) {
        ...
        if (task) {
            ...
            throw new HttpException('ไม่มี Task ให้ลบ', HttpStatus.BAD_REQUEST);
        }
    }
}
